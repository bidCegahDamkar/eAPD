...
class Auth extends CI_Controller
{
	...
	public function index()
	{
		...
		else if (!$this->ion_auth->is_admin()) // remove this elseif if you want to enable this for non-admins
		{
			...
			$jab_id = $this->ion_auth->user()->row()->jabatan_id;
			$controller = $this->auth_model->get_controller($jab_id);
			if(is_array($controller) ) {
				redirect($controller['controller'], 'refresh');
			} else {
				redirect('auth/login', 'refresh');
			}
		...
		else
		{
			...
			*/
			...
			//d($user);
			$jab_id = $this->ion_auth->user()->row()->jabatan_id;
			$user_tipe = $this->auth_model->get_userTipe($jab_id);
			//d($user_tipe);
			//d(is_int($user_tipe['tipe_user_id']));
			//d($user_tipe['tipe_user_id'] === '5');
			$this->_render_page('auth' . DIRECTORY_SEPARATOR . 'index');
