...
class Danton extends CI_Controller {
    ...
    public function __construct()
	{
        ...
        //$this->data['kode_pos'] = $user->kode_pos;
        ...
        $this->data['group_piket_id'] = $user->group_piket_id;
        $grupPiket = $this->petugas_model->get('kode_piket', 'master_group_piket', [['id', $this->data['group_piket_id']]], null, 2);
        $this->data['group_piket'] = $grupPiket['kode_piket'];
   ...
   public function index()
    ...
    public function home()
    {
        ...
        foreach($listEs as $es){
            if (! is_null($es['NRK'])) {
                $userid = $this->petugas_model->get('id', 'users', [['NRK', $es['NRK']]], null, 1);
                if (count($userid)>1) {
                    $duplicate[] = $es;
                } else if(count($userid) == 0) {
                    $dataMacth = ['macth' => 0];
                    $this->petugas_model->updateData('master_eselon', ['id', $es['id']], $dataMacth);
                    $nomach[] = $es;
                } else if(count($userid) == 1){
                    $data = array(
                        'NIP' => $es['NIP'],
                        'nama' => $es['nama'],
                        'jabatan_id' => $es['kode_jabatan'],
                        'kode_pos' => $es['kode_panggil'],
                    );
                    if( $this->petugas_model->updateData('users', ['id', $userid[0]['id']], $data) ){
                        $sukses[] = $es;
                    }else{
                        $nosukses[] = $es;
                    }
        ...
        $this->data['updateUser'] = [$duplicate, $nomach, $sukses, $nosukses];*/
         // id_mj kepala regu -> 102  insertData($tableName, $data)
        $select = 'nama, NRK, NIP, jabatan, kode_pos, no_telepon';
        $default_join = [['users', 'master_jabatan', 'master_jabatan.nama_jabatan', 'jabatan_id', 'id_mj' ]];
        //$where = [['master_jabatan.mc_id', 10]];
        //$where = [['jabatan', 'KR']];
        //$listEselon = $this->admin_model->get($select, 'tbl_pegawai', 1, $where);
        /*foreach ($listEselon as $eselon) {
            $data = [   'nrk' => $eselon['NRK'],
                        'nip' => $eselon['NIP'],
                        'nama' => $eselon['nama'],
                        'jabatan_id' => 102,
                        'kode_pos' => $eselon['kode_pos'],
                        'no_telp' => $eselon['no_telepon']
                    ];
            if ($this->admin_model->insertData('tabel_karu_katon', $data)) {
                $failed = 'success';
            } else {
                $failed = 'failed';
            }
    ...
    */
    public function monitoring()
    {
        ...
        if (is_null($search)) {
            $list_bawahan = $this->_get_users($select, 1, [['group_piket_id', $this->data['group_piket_id']]], [['kode_pos', $this->data['kode_pos'], 'after']], $or_where_arr, null, $join, ['master_controller.level', 'DESC']);
            $this->data['section_tittle'] = 'Menampilkan '.count($list_bawahan).' data petugas';
            /*$list_bawahan = $this->my_apd->get_list_user('users.id, nama, NRK, NIP, photo, users.no_telepon, email', 1, [['group_piket', $this->data['group_piket']]], 
                            [['users.kode_pos', $this->data['kode_pos'], 'after']], $or_where_arr);*/
        } else {
            $or_like = [['users.nama', $search], ['users.NRK', $search], ['master_jabatan.nama_jabatan', $search], ['master_pos.nama_pos', $search] ];
            $list_bawahan = $this->_get_users($select, 1, [['group_piket_id', $this->data['group_piket_id']]], [['kode_pos', $this->data['kode_pos'], 'after']], $or_where_arr, $or_like, $join, ['master_controller.level', 'DESC']);
            /*$list_bawahan = $this->my_apd->get_list_user('users.id, nama, NRK, NIP, photo, users.no_telepon, email', 1, [['group_piket', $this->data['group_piket']]], 
                                [['users.kode_pos', $this->data['kode_pos'], 'after']], $or_where_arr,
                                 [['users.nama', $search], ['users.NRK', $search], ['master_jabatan.nama_jabatan', $search],
                                ['master_pos.nama_pos', $search] ] );*/
    ...
    public function report_rekap_APD()
    {
        ...
        $list_jab_id_bawahan = $this->config->item('list_jab_id_bawahan');
        foreach ($jenisApd as $apd) {
            $akronim[] = $apd['akronim'];
            $jenis_apd[] = $apd['jenis_apd'];
            $temp_belum = $this->my_apd->get_report($apd['id_mj'], 'belum', null, $this->data['periode'], $this->data['group_piket'], $this->data['kode_pos'], $list_jab_id_bawahan);
            $temp_hilang = $this->my_apd->get_report($apd['id_mj'], 'hilang', null, $this->data['periode'], $this->data['group_piket'], $this->data['kode_pos'], $list_jab_id_bawahan);
            $temp_baik = $this->my_apd->get_report($apd['id_mj'], null, 4, $this->data['periode'], $this->data['group_piket'], $this->data['kode_pos'], $list_jab_id_bawahan);
            $temp_rr = $this->my_apd->get_report($apd['id_mj'], null, 3, $this->data['periode'], $this->data['group_piket'], $this->data['kode_pos'], $list_jab_id_bawahan);
            $temp_rs = $this->my_apd->get_report($apd['id_mj'], null, 2, $this->data['periode'], $this->data['group_piket'], $this->data['kode_pos'], $list_jab_id_bawahan);
            $temp_rb = $this->my_apd->get_report($apd['id_mj'], null, 1, $this->data['periode'], $this->data['group_piket'], $this->data['kode_pos'], $list_jab_id_bawahan);
            $belum[] = $temp_belum;
            $hilang[] = $temp_hilang;
        ...
        $this->data['result'] = array('title' => 'Laporan data APD', 'group' => $this->data['penempatan']['nama_pos'].', Pleton '.$this->data['group_piket'], 
        ...
        //$jmlPrsnl = $this->admin_model->get('id', 'users', 3, [['active', 1], ['group_piket', $this->data['group_piket'] ]], [['kode_pos', $this->data['kode_pos'], 'after']], null, null, null, $or_where_arr );
        $this->data['jmlAPDTervld'] = array_sum($stot);
        $this->data['jmltotalAPD'] = $jmlPrsnl*count($jenisApd);
