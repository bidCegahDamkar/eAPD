...
$('.custom-file-upload input[type="file"]').each(function () {
    ...
    $fileUpload.on('change', function (event) {
        var name = $fileUpload.val().split('\\').pop(),
            ...
            tmppath = URL.createObjectURL(img);
        ...
        new Compressor(img, {
            ...
            success(result) {
                ...
                reader_img.readAsDataURL(result);
                reader_img.onloadend = function() {
                    var base64data = reader_img.result;
        ...
        new Compressor(img, {
            ...
            success(result) {
                var reader_thumb = new FileReader();
                reader_thumb.readAsDataURL(result);
                reader_thumb.onloadend = function() {
                    var base64data1 = reader_thumb.result;
                    $('#thumb-temp').val(base64data1);
                }
            },
        });
