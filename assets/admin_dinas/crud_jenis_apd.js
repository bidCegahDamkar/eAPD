    var table_kondisi = $('#list-merk').DataTable( {
        dom: 'Bfrtip',
        lengthMenu: [
            [ 10, 25, 50, -1 ],
        ...
        buttons: [
            ...
            ,{
                ...
                exportOptions: {
                    columns: [ 1, 2]},
                title: 'Daftar Master Merk APD --generated by eAPD'
            }
            ,{
                extend: 'pdfHtml5',
                exportOptions: {
        ...
        "order": [[1, "asc" ]],
        "ajax": {
            url :  base_url+controller+'/list_merk_datatables',
            type : 'GET',
    } } );
    ...
    function add_merk()
    {
        get_csrf();
        save_method = 'add';
        $('#form_merk')[0].reset(); // reset form on modals
        $('.form-select').removeClass('is-valid'); // clear error class
        $('.form-select').removeClass('is-invalid');
        $('.help-block').empty(); // clear error string
        $('#btnSaveMerk').text('Simpan');
        $('#modalMerk').modal('show'); // show bootstrap modal
        $('.modal-title').text('Tambah Data Merk'); // Set Title to Bootstrap modal title
    }
    ...
    function edit_merk(id)
    {
        ...
        $.ajax({
            ...
            success: function(result)
            {
                ...
                $('#modalMerk').modal('show'); // show bootstrap modal when complete loaded
    ...
    function save()
    {
        ...
        $('#btnSaveMerk').text('Menyimpan...'); //change button text
        $('#btnSaveMerk').attr('disabled',true); //set button disable 
        var url,info;
        //validasi();
        if(save_method == 'add') {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                $('#btnSaveMerk').text('Simpan'); //change button text
                $('#btnSaveMerk').attr('disabled',false); //set button enable 
